...
<tbody>
<%
		BbsDAO regularDAO = new BbsDAO();
		ArrayList<Bbs> list = regularDAO.getList(pageNumber);
		for(int i = 0; i < list.size(); i++){
			String link =  list.get(i).getBbsURL();
			%>
<tr>
<td><%= list.get(i).getBbsID() %></td>
<td><a href="view_normal.jsp?bbsID=<%= list.get(i).getBbsID() %>"><%= list.get(i).getBbsTitle() %></a></td>
<td>
...
if(pageNumber != 1){
%>
 <a href="normalbbs.jsp?pageNumber=<%=pageNumber -1 %>" class="btn btn-success btn-arrow-left">이전</a>
 <%
} if(regularDAO.nextPage(pageNumber + 1)){
 %>
 <a href="normalbbs.jsp?pageNumber=<%=pageNumber +1 %>" class="btn btn-success btn-arrow-left">다음</a>
 <%
}
 %>
<a href="write_normal.jsp" id="writebutton" class="btn btn-primary  bg-info" style="float:right;">글쓰기</a>
</div>	
